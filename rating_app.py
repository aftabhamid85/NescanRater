...
# Define the 10 questions and their scale labels (from PDF, with mid-labels for 5)
questions = [
    {
...
if st.session_state.rater_name:
    ...
    if current_index >= total_orgs:
        ...
        # Display sliders with custom labels at 1, 5, 10
        ratings = {}
        for q in questions:
            st.subheader(q["text"])
            rating = st.slider(
                ...
                value=None,  # No pre-filling
                step=1,
                format="%d",
                key=f"slider_{q['text']}",
                help=f"{q['label_1']} (1) | {q['label_5']} (5) | {q['label_10']} (10)"
            )
    ...
    if not st.session_state.ratings.empty:
        ...
        st.download_button(
            ...
            data=st.session_state.ratings.to_excel(index=False, engine='openpyxl'),
